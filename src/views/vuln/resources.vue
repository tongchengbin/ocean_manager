        ...
        <el-table  fit highlight-current-row stripe v-loading="loading" :data="listData">
          ...
          <el-table-column
            ...
            <template #default="scope">
              ...
              <el-button v-else  type="primary" @click="runHandle(scope.row)" link>启动</el-button>
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      http.get(`/api/admin/vuln`, this.listQuery).then(res => {
    ...
    handleFileUpload(event) {
      ...
      http.post('/api/admin/vuln/import',formData).then(res=>{
    ...
    runHandle(row){
      http.post(`/api/admin/vuln/${row.id}/run`,).then(_ => {
        this.$message({
          message: "启动成功",
          type: "success"
        })
    ...
    deleteHandle(row) {
      http.delete(`/api/admin/vuln/${row.id}`,).then(_ => {
        this.$message({
          message: "删除成功",
          type: "success"
        })
