<template>
  <el-dialog :before-close="cancel" :show-close="true" :title="form.id ? '编辑题目' : '添加题目'" v-model="visible" width="40%">
    <el-form :model="form" ref="form" class="form" label-width="80px" :rules="rules">
      ...
      <el-form-item  v-if="form.active_flag" class="block-input" label="环境" prop="resource_id">
        <el-select v-model="form.resource_id" clearable filterable>
          <el-option v-for="item in resource_list" :key="item.id" :label="item.name" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        flag: [
          { validator: this.validateFlagOrResource, trigger: "blur" },
  ...
  methods: {
    ...
    submit() {
      ...
      this.$refs["form"].validate((valid, err) => {
        if (valid) {
          if (this.form.id) {
            http
              .put(`/api/admin/ctf/question/${this.form.id}`, this.form)
              .then((_) => {
            ...
            http
              .post("/api/admin/ctf/question", this.form)
              .then((_) => {
                this.$emit("action", true);
              })
              .catch((_) => {});
          ...
          this.$message({ message: "表单验证失败:" + JSON.stringify(err), type: "error" });
        }
      });
    },
    handleFileClick() {
    ...
    handleFile() {
      ...
      http
        .post("/api/admin/ctf/upload", formData, {
    ...
    validateFlagOrResource(rule, value, callback) {
      if (!this.form.flag && !this.form.resource_id) {
        callback(new Error("Flag 和 Resource ID 至少填写一个"));
      } else {
        callback();
      }
    },
