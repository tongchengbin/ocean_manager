        ...
        <el-table v-loading="loading"  :data="data" highlight-current-row>
          ...
          <el-table-column align="center" label="操作" width="160">
            <template #default="scope">
              ...
              <el-button  type="primary" @click="refresh(scope.row)">刷新</el-button>
            </template>
          </el-table-column>
        </el-table>
        <div class="page-r">
...
export default {
  ...
  methods: {
    ...
    refresh(item) {
      http.post(`/api/admin/ctf/resource/${item.id}/refresh`).then(res => {
        this.getList()
      }).catch(err => {
    ...
    containerRemove(item) {
      http.post(`/api/admin/ctf/resource/${item.id}/remove`).then(res => {
